...
## Current live setup (as of June 2026)

The site is **already live** at https://cosilab.ucsf.edu, served by **GitHub
Pages** from the `main` branch (root) of `midnightmd2/cosilab`. The `CNAME`
file and the GitHub Pages custom-domain setting are both set to
`cosilab.ucsf.edu`. Because that is a UCSF subdomain, its DNS is managed by
UCSF (a CNAME pointing `cosilab` at `midnightmd2.github.io`), not by the
apex A-records described in Route B below.

**To update the site:** edit a file and push to `main` (or merge a PR into it);
Pages rebuilds within a minute. Non-developers can edit all page text and
upload member photos through the Pages CMS at https://app.pagescms.org.

The routes below are kept as reference for moving the site elsewhere.
...
5. In Netlify: Site configuration → Domain management → Add a domain →
   type `cosilab.ucsf.edu` → Verify → Add.
...
### Connect cosilab.ucsf.edu
8. Settings → Pages → Custom domain → enter `cosilab.ucsf.edu` → Save.
   (The included `CNAME` file already sets this, so it may be filled in.)
9. At your domain registrar's DNS settings, add:

   Apex domain (cosilab.ucsf.edu) — four A records:
   - `185.199.108.153`
   - `185.199.109.153`
   - `185.199.110.153`
   - `185.199.111.153`
